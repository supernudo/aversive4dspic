...
#else
	sprintf(debug_buf, "%s", buf); // XXX take care, buffer overruns?
#endif
...
#else
				sprintf(dst, PSTR("[RETURN]")); // XXX take care, buffer overruns?
#endif
			}
			return 1;
		}
...
#else
				l=sprintf(dst, "%s", tmpbuf); // XXX take care, buffer overruns?
#endif
